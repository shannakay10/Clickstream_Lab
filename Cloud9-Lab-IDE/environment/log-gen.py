# This one is very close, but has one gap.
# It currently does not support multiple vistors puchasing multiple items.  
...
import zipfile
import sys
# this next line fixes issues with modules pytz, numpy and tzlocal missing.  Keep for EAR, and find better solution for golive.
...
# You can use the original ualist above, but it will provide far too many useragent types and becomes hard to visualize at scale.  So, this was reduced to 4 types, as manually assigned to the variables.

# first iPhone, then Android, then PC, then Mac
...
flag = True
while (flag):
    # with the code below, timedelta is not enough for reporting.
    # if args.sleep:
        # increment = datetime.timedelta(seconds=args.sleep)
    # else:
        # increment = datetime.timedelta(seconds=random.randint(30, 300))
    # Therefore refactoring with seconds, but changing to random increment of 0 or 1 second and will test the results.
    if args.sleep:
        increment = datetime.timedelta(seconds=args.sleep)
    else:
    ...
    if log_format == "CLF":
        ...
        f.write('{ "time":"%s", "process":"%s", "filename":"%s", "remoteIP":"%s", "host":"%s", "request":"%s", "query":"", "method":"%s", "status":"%s", "useragent":"%s", "referer":"%s"}\n' % (dt,byt,fn,ip,ipAddress,uri,vrb,resp,useragent,referer))        
    f.flush()
...
# Original log format
    #f.write('%s - - [%s %s] "%s %s HTTP/1.0" %s %s "%s" "%s" "%s" "%s" "%s"\n' % (ip,dt,tz,vrb,uri,resp,byt,referer,useragent,geo_country,geo_lat,geo_long))

# New log format without geo location
    #f.write('{ "time":"%s", "process":"%s", "filename":"%s", "remoteIP":"%s", "host":"%s", "request":"%s", "query":"", "method":"%s", "status":"%s", "useragent":"%s", "referer":"%s"}\n' % (dt,byt,fn,ip,ipAddress,uri,vrb,resp,useragent,referer))        

# New format with geo location
    #f.write('{ "time":"%s.%s", "process":"%s", "filename":"%s", "remoteIP":"%s", "host":"%s", "request":"%s", "query":"", "method":"%s", "status":"%s", "useragent":"%s", "referer":"%s", "country":"%s", "region":"%s", "city":"%s", "lat": "%s", "lon":"%s"}\n' % (dt,tz,byt,fn,ip,ipAddress,uri,vrb,resp,useragent,referer,geo_country,geo_region,geo_city,geo_lat,geo_long))  
    
# Orignal dt calculation
